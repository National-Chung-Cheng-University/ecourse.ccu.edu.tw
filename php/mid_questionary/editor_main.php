<?
	...
	function delitem () { //刪除題目
		global $version,  $item_id, $q_id;
		global $DB_SERVER, $DB_LOGIN, $DB, $DB_PASSWORD;
		$Q1 = "delete from mid_question where a_id = '$item_id'";
	...
	function updateitem() {  //修改題目
		global $version,  $selection1, $selection2, $selection3, $selection4, $selection5, $qgrade, $qtext, $cho, $rownum, $q_id, $block_id, $type, $item;
		global $DB_SERVER, $DB_LOGIN, $DB, $DB_PASSWORD;
		if( $qtext == "" ) {
			show_page_d( "6" );
			exit;
		...
		if ( $type == 1 ) {
			//2010.10.13 update by JIM for 複選題,加入ismultiple欄位
			$Q1 = "update mid_question set question = '$qtext', selection1 = '$selection1',  selection2 = '$selection2', selection3 = '$selection3', selection4 = '$selection4', selection5 = '$selection5', note='$rownum', ismultiple = '$cho' where a_id = '$item'";
		}
		else if ( $type == 2 ) {
			$Q1 = "update mid_question set question = '$qtext' where a_id = '$item'";
		}
		else {
	...
	function additem () {  //增加題目
		global $version,  $selection1, $selection2, $selection3, $selection4, $selection5, $qtext, $cho, $rownum, $q_id, $block_id, $type, $year, $term;
		global $DB_SERVER, $DB_LOGIN, $DB, $DB_PASSWORD;
		if( $qtext == "" ) {
			show_page_d( "6" );
			exit;
		...
		if ( $type == 1 ) {
			//2010.10.13 update by JIM for 複選題,加入ismultiple欄位
			$Q1 = "Insert into mid_question ( q_id, block_id, type, question, selection1, selection2, selection3, selection4, selection5, note, ismultiple ) values ( '$q_id', '$block_id', '$type', '$qtext','$selection1','$selection2','$selection3','$selection4','$selection5', '$rownum', '$cho')";
		}
		else if ( $type == 2 ) {
			$Q1 = "Insert into mid_question ( q_id, block_id, type, question ) values ( '$q_id', '$block_id', '$type', '$qtext')";
		}
		else {
			show_page( "not_access.tpl" ,"題型錯誤!!" );
			exit;
		...
		if ( !($result = mysql_db_query( $DB, $Q1 ) ) ) {
			show_page( "not_access.tpl" ,"資料庫寫入錯誤3!!" );
		}
		$qtikuid = mysql_insert_id();
		/*$Q3 = "SELECT test.course_id FROM test WHERE test.year = '$year' AND test.term = '$term' "; //選擇此學期的課程id
			if ( !($result2 = mysql_db_query( $DB.$row2['course_id'], $Q3 ) ) ) {
				show_page( "not_access.tpl" ,"資料庫讀取錯誤4!!" );
			} 
		while( $row2 = mysql_fetch_array($result2) ){   //此學期課程的mid_ans table 皆增加一題欄位(IF NOT EXISTS)
			if ( $type == 1 ) {
				$Q3 = "alter table mid_ans add q$qtikuid tinytext after term";
				if ( !($result3 = mysql_db_query( $DB.$row2['course_id'], $Q3 ) ) ) {
					show_page( "not_access.tpl" ,"資料庫寫入錯誤5!!" );
				}
			}
			else if ( $type == 2) {
				$Q3 = "alter table mid_ans add q$qtikuid text after term";
	...
	function newblock () { //left樹狀區塊
		global $DB_SERVER, $DB_LOGIN, $DB, $DB_PASSWORD;
		global $q_id,  $block_title;
		mysql_pconnect($DB_SERVER,$DB_LOGIN,$DB_PASSWORD) or die("資料庫連結錯誤");
		$Q1 = "Insert into mid_question ( q_id, type, question ) values ( '$q_id', '3', '$block_title')";
		$result = mysql_db_query($DB, $Q1) or die("資料庫寫入錯誤, $Q1");
		show_page_d( "0" );
	}

	function updateblock () {
		global $DB_SERVER, $DB_LOGIN, $DB, $DB_PASSWORD;
		global $q_id, $block_title, $block_id;
		mysql_pconnect($DB_SERVER,$DB_LOGIN,$DB_PASSWORD) or die("資料庫連結錯誤");
		$Q1 = "update mid_question set question='$block_title' where q_id= '$q_id' and a_id ='$block_id'";
		$result = mysql_db_query($DB, $Q1) or die("資料庫更新錯誤, $Q1");
		show_page_d( "2" );
	}

	function delblock () {
		global $DB_SERVER, $DB_LOGIN, $DB, $DB_PASSWORD;
		global $q_id,  $block_id;
		mysql_pconnect($DB_SERVER,$DB_LOGIN,$DB_PASSWORD) or die("資料庫連結錯誤");
		$Q1 = "delete from mid_question where a_id = '$block_id'";
		$result = mysql_db_query($DB, $Q1) or die("資料庫寫入錯誤, $Q1");
		show_page_d( "1" );
	}

	function show_page_d ( $errno="" ) {  //顯示頁面

		include("class.FastTemplate.php3");
		global $block_id, $item, $q_id, $version, $reload, $bno, $type, $rownum, $qno, $qgrade;
		if($version == "C") {
		...
		if(isset($block_id) && empty($item)) {         
			...
			$sql = "select * from mid_question where block_id='$block_id' and type != '3' and q_id = '$q_id' order by a_id";
			...
			//題型新增
			$Q1 = "SELECT * FROM mid_question WHERE q_id = '$q_id' and block_id = '$block_id' and type != '3'";
			if ( !($result = mysql_db_query( $DB, $Q1 ) ) ) {
			...
			$sql = "select * from mid_question where a_id = '$item'";
			$result = mysql_db_query($DB, $sql) or die("資料庫查詢錯誤, $sql");
			...
			$sql = "select * from mid_question where type='3' and q_id = '$q_id' order by a_id";
			$result = mysql_db_query($DB, $sql) or die("資料庫查詢錯誤, $sql");
