<?php
	...
	function show_page_t(){
		...
		if ( $check == 2 ) {
			...
			if ( $version == "C" )
				$tpl->define ( array ( body => "talk_ints.tpl" ) );
			else
				$tpl->define ( array ( body => "talk_ints_E.tpl" ) );
			
			$tpl->assign( PHPSID, $PHPSESSID );
			$tpl->assign( CNAME, $row2['name'] );
			$tpl->assign( IPS, $ip );
			$tpl->assign( CID, $topic );
		}
		else {
			if ( $version == "C" )
				$tpl->define ( array ( body => "talk_intc.tpl" ) );
			else
				$tpl->define ( array ( body => "talk_intc_E.tpl" ) );
			...
			$tpl->assign( SERVER, $row3['tag1'] );
			$tpl->assign( CID, $row3['tag4'] );
			if ( $row4['name'] != NULL )
				$tpl->assign( TNAME, $row4['name'] );
			else if ( $row4['nickname'] != NULL )
				$tpl->assign( TNAME, $row4['nickname'] );
			else
				$tpl->assign( TNAME, $row4['id'] );
		}
		if ( $row['name'] != NULL )
			$name = $row['name'];
	...
	function show_page_p () {
		...
		$tpl = new FastTemplate ( "./templates" );
		if ( $version == "C" )
			$tpl->define ( array ( body => "talk_int.tpl" ) );
		else
			$tpl->define ( array ( body => "talk_int_E.tpl" ) );
