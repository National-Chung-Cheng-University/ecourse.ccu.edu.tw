...
while($row = mysql_fetch_array( $result1)){  // gid
  $groupid = $row['a_id'];
		...
		$q2="SELECT teach_course.*,course.name FROM `teach_course`,`course` WHERE teach_course.course_id=course.a_id and teach_course.teacher_id=$row2[teacher_id] and name=\"$row2[cname]\" and year=93 and term=2";
