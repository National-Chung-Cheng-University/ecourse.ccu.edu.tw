...
//		chap , sect for redirect
			...
			while($row = mysql_fetch_array($result)) {
				...
				if(mysql_num_rows($result2) != 0) {
					...
					while($row2 = mysql_fetch_array($result2)) {
						...
						$sect_title = str_replace('"','\"',$sect_title);
	...
	function show_file_list( $dir_name ){
		...
		foreach($files as $file)
		{
			if( ( strcmp($file,".")!=0 ) && ( strcmp($file,"..")!=0 ) && !is_dir($dir_name."/".$file) ) {
				// 除了 '.', '..', 目錄外的檔案輸出
				//$calais = rawurlencode("&");					//---處理特殊字元
				//$file = str_replace ( "&", $calais , $file );	
				//$calais = rawurlencode("#");
				//$file = str_replace ( "#", $calais , $file );				
				//$tpl->assign("FILE_N", rawurldecode($file));
				$tpl->assign("FILE_N", $file);
				//$tpl->assign("FILE_LINK", $dir_name."/".$file);
				$tpl->assign("FILE_LINK", $dir_name."/".rawurlencode($file));	//--by j.p 20070919
				//$tpl->assign("FILE_SIZE", filesize($dir_name."/".stripslashes(rawurldecode($file))));
				$tpl->assign("FILE_SIZE", filesize($dir_name."/".stripslashes($file)));
				//$tpl->assign("FILE_DATE", date("Y-m-d H:i:s",filemtime(rawurldecode($dir_name."/".$file))));
