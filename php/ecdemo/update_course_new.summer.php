...
function update_course($sybase_name)
{
	...
	//update line 98.06.26	
	$cur = sybase_query("select a.year, a.term, a.unitname, a.cour_cd, a.grp, b.name, a.id from a31vcurriculum_summer_tea a, a30vcourse_tea b where a.cour_cd=b.course_no and a.year='97' and a.term='3'" , $cnx);
	if(!$cur) 
	{  
	...
	$testlog_content = $date_info."更新".$sybase_name."，總共".$total ."堂課程教師資料\n";
	...
	{	
		...
		{	
			...
			fwrite($testlog_fp,$testlog_content);								
			update_coursename($cid, $array["name"], $array["unitname"]);//更新課程名稱
		...
		{	
			...
			{	
				...
				//開課教師為多人
				$q_num = "select count(a.id) as num from a31vcurriculum_summer_tea a where a.year='".$array['year']."' AND a.term='".$array['term']."' AND a.cour_cd='$array[cour_cd]' AND a.grp='$array[grp]'";
...
function del_teach_course($cnx, $sybase_name)
{
	global $DB;
	$csd = @sybase_select_db($sybase_name, $cnx);
	$cur = sybase_query("SELECT DISTINCT year, term FROM a31vcurriculum_summer_tea" , $cnx);
	if(!$cur) 
	{ 
		Error_handler( "在 sybase_exec 有錯誤發生( 沒有指標傳回 ) " , $cnx );  
	}
	...
	{	
		...
		while($row = mysql_fetch_array($result)){
			...
			if($row["tid"]=="undefined"){
				$qtno = "SELECT * FROM a31vcurriculum_summer_tea WHERE year='$row[year]' AND term='$row[term]' AND cour_cd='$cid' AND grp='$gid' AND id='99999'";
			}
			else{
				$qtno = "SELECT * FROM a31vcurriculum_summer_tea WHERE year='$row[year]' AND term='$row[term]' AND cour_cd='$cid' AND grp='$gid' AND id='$row[tid]'";
			}
