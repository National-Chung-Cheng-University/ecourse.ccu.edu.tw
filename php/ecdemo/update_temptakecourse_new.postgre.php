...
/*	if ( isset($PHPSESSID) && session_check_admin($PHPSESSID) )
	...
	//linsy@20120629, 新增判斷sync欄位，值為1表示選課系統同步成功，0表示不成功
	if ( !($link = mysql_pconnect($DB_SERVER,$DB_LOGIN,$DB_PASSWORD)) )
	{
		die("資料庫連結錯誤!!");		
	}
	
	$Q0 = "select sync from temp_takecourse_status";
	...
	if($sync == '1')
	{
		...
		if(($error = update_temptakecourse()) == -1){
	  ...
	  echo "開始清除暫時性修課資料<BR>";
		...
		if(($error = del_now_termtakecourse()) == -1)
		{
			echo "暫時性選課清除完畢!!<br>";
	...
	$sql = "update temp_takecourse_status set sync='0'";
...
function del_old_takecourse()
{
  ...
  $db_name = 'academic';
	...
	$cur = pg_query($cnx, "select DISTINCT year, term from a31v_sel_class_tea") or die('資料表不存在，請通知電算中心');
	
	
	//if($array=sybase_fetch_array($cur))
	if($array=pg_fetch_array($cur, null, PGSQL_ASSOC))
	{
		$year = $array['year'] - 1;
		$term = $array['term'];
	}
...
// 更新選課資料
function update_temptakecourse()
{
	global $DB_SERVER, $DB_LOGIN, $DB, $DBC, $DB_PASSWORD;
	// 連結mysql
	if ( !($link = mysql_pconnect($DB_SERVER,$DB_LOGIN,$DB_PASSWORD)) )
...
// 98.07.06	增加192~206 暑修第3學期選課
	...
	$Q0 = "select student_id, course_no from temp_takecourse order by course_no";
	$result0 = mysql_db_query($DB, $Q0);
	if(!$result0)
	{
		echo "資料庫讀取錯誤!! $Q0<br>";
...
function del_now_termtakecourse()
{
	global $DB_SERVER, $DB_LOGIN, $DB, $DBC, $DB_PASSWORD;
	$delete = 0;
	// 連結mysql
...
// 98.07.06修改 暑修第3學期選課	
	...
	while ($row0 = mysql_fetch_array($result0))
	{
		...
		$Q3 = "select * from temp_takecourse where student_id='".$row2[id]."' and course_no='".$row1[course_no]."'";
		$Q4 = "select * from temp_takecourse where student_id='".$row2[id]."' and course_no='".$row1_1[course_no]."'"; //合併的課號
