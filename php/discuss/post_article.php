...
// modified by kof9x at 03/01/20, fix object bug.
	...
	if(empty($action)) {      // 發表新文章/回覆文章
		...
		$suppose_id = mt_rand();
		$param[3] = "<param name=\"UploadFile\" value =\"".$suppose_id.".gsm\">";
		// fixed param.
		...
		// modified by kof9x at 03/01/02, due to object bug cause IE crash.
		// modified by kof9x at 03/01/20, fix bug.
		// $tpl->assign("RECORDER_CODE", NULL);

		$tpl->assign("SOUNDNAME", $suppose_id.".gsm");
		...
		// 存放附加檔案
		if(is_file($attach)) {
			if( !fileupload( $attach, "../../$course_id/board/$discuss_id", "$this_id.$type" ) ){
		...
		if((session_check_teach($PHPSESSID) != 0) && ( strcmp( $user_id, "guest" ) !=0 ) ) {
			add_log( 6, $user_id, "", $course_id );
		}
