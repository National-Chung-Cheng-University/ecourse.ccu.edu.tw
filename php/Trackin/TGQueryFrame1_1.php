...
if(isset($PHPSESSID) && session_check_teach($PHPSESSID) == 2)
{
	...
	//取出該門課的學生
	$Q1 = "SELECT u.a_id, u.id, u.name, u.email FROM user u, take_course tc WHERE u.a_id=tc.student_id AND tc.course_id='$course_id' and tc.credit = '1' and tc.year='$course_year' and tc.term = '$course_term' ORDER BY u.id";
...
//				$scordata[$j]['email']	= $row1[3];//email
				...
				while ( $row3 = mysql_fetch_array($result6) ) {
					...
					if ( $row3[0] != "" && $row3[0] != "-1" && $row3[0] != null) {//20070615jp-- 0%會顯示
			...
			if($action == "upload"){
				...
				for($i=0;$i<sizeof($scordata);$i++){
					$std_no = $scordata[$i][0];
					if( $scordata[$i][sizeof($scordata[$i])-1] >='0' ){//20070117jp,with socres  
				...
				header("Location:TGShowFrame.php?PHPSESSID=$PHPSESSID");
			}
			//--
...
function do_update () {
	global $DB_SERVER, $DB_LOGIN, $DB, $DBC, $DB_PASSWORD, $mid, $maid, $work, $grade, $course_id;
	...
	$Q0 = "select grade from take_exam where exam_id='$maid' and student_id='".$row['a_id']."'";
	$result0 = mysql_db_query($DB.$course_id, $Q0);
	$num = mysql_num_rows($result0);
	if($num !=0)
	{
		if ( $work == 1 ) {
			$Q2 = "update handin_homework set grade = '$grade' where homework_id = '$maid' and student_id = '".$row['a_id']."'";
		}
	/*
	else if ( $work == 2 ) {
	...
	*/
			...
			$Q2 = "update take_exam set grade = '$grade' where exam_id = '$maid' and student_id = '".$row['a_id']."'";
			$Q3 = "select * from exam where is_online = '0' and a_id = '$maid'";
			($result3 = mysql_db_query( $DB.$course_id, $Q3 )) or die ("資料庫圖取錯誤!!");
	...
	//else {
		mysql_db_query( $DB.$course_id, $Q2 ) or die ("資料庫更新錯誤!!");
	//}
	}
	else
	{
	...
	*/
			...
			$Q2 = "insert into take_exam (grade, exam_id, student_id) values ('$grade', '$maid', '$row[a_id]')";// = '$grade' where exam_id = '$maid' and student_id = '".$row['a_id']."'";
			$Q3 = "select * from exam where is_online = '0' and a_id = '$maid'";
			($result3 = mysql_db_query( $DB.$course_id, $Q3 )) or die ("資料庫圖取錯誤!!");
			if ( mysql_num_rows( $result3 ) == 0 ) {
				return;
	...
	//else {
		mysql_db_query( $DB.$course_id, $Q2 ) or die ("資料庫更新錯誤22!!");
	}
}

function show_page_d () {
	global $DB_SERVER, $DB_LOGIN, $DB, $DBC, $DB_PASSWORD, $version, $message, $course_id, $scordata, $countdata, $sel, $skinnum, $c_id, $a_id, $id, $action, $work, $PHPSESSID;
	global $mail;
	if ( !($link = mysql_pconnect($DB_SERVER,$DB_LOGIN,$DB_PASSWORD)) ) {
		show_page( "not_access.tpl" ,"資料庫連結錯誤!!" );
	}
	...
	*/
	...
	if ( $version == "C" )
		$tpl->define(array(main=>"TGQueryFrame1.tpl"));
	else
		$tpl->define(array(main=>"TGQueryFrame1_E.tpl"));
	...
	*/
	...
	for ( $j = 0 ; $j < $countdata[0][0]; $j ++ )
	{
		...
		$c_index = 0; //by carlyle
		for($k = 0; $k <= $countdata[0][1] + 2; $k ++,$c_index++ )
		{
			if($k == 0 || $k == 1 ) {
				if ( $k == 0 ) {
					//排名
					...
					$tpl->assign(DATA,"<tr bgcolor = $color onmouseover=\"set_obj( this, '$j')\" onmousedown=\"rang_bg('start', '$j')\" onmouseup=\"rang_bg('end', '$j')\"><td><div align=\"center\"><font size=\"2\">$i</font></div></td><td><div align=\"center\"><font size=\"2\"><A HREF=mailto:#>".$scordata[$j][$k]."</A></font></div></td>");
				...
				if ( $k == $countdata[0][1] + 2 ) {
					...
					if ( $countdata[5][$k] == 0 ) {
						if ( $action == "modify_single" && $countdata[6][$k] == $a_id && $scordata[$j][0] == $id && $c_id == $c_index ) {
							$tpl->assign(DATA,"<form action=./TGQueryFrame1.php method=post><td><input type=hidden name=mid value=".$scordata[$j][0]." ><input type=hidden name=work value=$work><input type=hidden name=maid value=".$countdata[6][$k]."><input type=hidden name=action value=update><input type=text name=grade value=\"".$scordata[$j][$k]."\" size=2><input type=submit value=\"送出\" ></td></form>");
						}
						else if ( $scordata[$j][$k] == " " ) {
							$tpl->assign(DATA,"<td><div align=\"center\"><a href=\"./TGQueryFrame1.php?action=modify_single&c_id=".$c_index."&a_id=".$countdata[6][$k]."&id=".$scordata[$j][0]."&work=".$countdata[7][$k]."\" >.</a></div></td>");
						}
						else {
							if($scordata[$j][$k] < 60)
								$tpl->assign(DATA,"<td><div align=\"center\"><a href=\"./TGQueryFrame1.php?action=modify_single&c_id=".$c_index."&a_id=".$countdata[6][$k]."&id=".$scordata[$j][0]."&work=".$countdata[7][$k]."\" ><font color=#ff0000 size=\"2\">".$scordata[$j][$k]."</font></a></div></td>");
							else
								$tpl->assign(DATA,"<td><div align=\"center\"><a href=\"./TGQueryFrame1.php?action=modify_single&c_id=".$c_index."&a_id=".$countdata[6][$k]."&id=".$scordata[$j][0]."&work=".$countdata[7][$k]."\" ><font size=\"2\">".$scordata[$j][$k]."</font></a></div></td>");
						}
					}
					else {
						if($scordata[$j][$k] < 60)
	...
	var_export($countdata);
	$tpl->assign(LOCATION,"$course_id/score_$course_id.xls");
	$tpl->parse(BODY,"main");
	$tpl->FastPrint("BODY");
}
