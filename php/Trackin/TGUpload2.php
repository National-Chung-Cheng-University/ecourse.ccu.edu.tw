...
function upload_score(){
	...
	for($i=0;$i<sizeof($report);$i++){
		if($_POST["grade".$report[$i]['std_no']]!="" && $_POST["state".$report[$i]['std_no']]==1){
			$std_no = $report[$i]['std_no'];
			//100.4.26 add by Jim for 校際選課生修改
			//echo $report[$i]['sch_no'];
			$sch_no = $report[$i]['sch_no'];
			IF ($sch_no == "CCU")
			{
				$sch_no = "";
			}
			...
			$Qins = "insert into a32t_sel_score_t (year, term, cour_cd, grp, std_no, trmgrd, ipaddr, sysdate, sch_no) values ('$course_info[year]', '$course_info[term]', '$course_info[cid]', '$course_info[gid]', '$std_no', $grade, '$ip', '$date', '$sch_no' )";
			//$cur = sybase_query($Qins, $cnx);	//測試用..不上傳--jp
			//if(!$cur) {  
			//	Error_handler( "在 sybase_exec 有錯誤發生( 沒有指標傳回 ) " , $cnx );  
			//}
			$cur = pg_query($cnx, $Qins ) or die('資料表不存在，請通知電算中心');	//測試用，不上傳，如需要時再開啟--jp96
...
function show_page_d($page){
	...
	for($i=0;$i<pg_num_rows($cur);$i++){
		...
		$data[$i]['name'] = mb_convert_encoding($row['name'], "big5", "utf-8");
		...
		//	Error_handler( "在 sybase_exec 有錯誤發生( 沒有指標傳回 ) " , $cnx );  
		//}
		$cur2 = pg_query($cnx, $Qs2 ) or die('資料表不存在，請通知電算中心');
		// 查詢棄選資料
...
/*********************************************************************************************************************************/
		...
		else if(pg_fetch_array($cur3, null, PGSQL_ASSOC)!=0){
			$data[$i]['score'] = "棄選";	//若有棄選資料則不論有無成績都標為棄選
		}
		//else if(($row = sybase_fetch_array($cur2))!=0){
		else if(($row = pg_fetch_array($cur2, null, PGSQL_ASSOC))!=0){
		...
		*/
		...
		if(($row3 = pg_fetch_array($cur3, null, PGSQL_ASSOC))!=0){
			...
			$data[$i]['grp'] = mb_convert_encoding($row3['abbrev'], "big5", "utf-8").$row3['now_grade'].$row3['now_class'];
		}
		else{
