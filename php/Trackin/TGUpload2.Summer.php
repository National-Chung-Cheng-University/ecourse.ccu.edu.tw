<?php
//devon@2006-01-10-------為了讓老師能不用進入系統就能上傳成績至教務處
require 'fadmin.php';
global $count, $action;
...
if(isset($PHPSESSID) && (session_check_teach($PHPSESSID) ==0 || session_check_teach($PHPSESSID) == 2) ){
	...
	{
		if( $version=="C" ) {
			show_page( "not_access.tpl" ,"你沒有權限使用此功能，請冾正式教師,1");
			exit;
		}
		else {
			show_page( "not_access.tpl" ,"You have No Permission!!Please Contact With Your Teacher!!,1");
			exit;
		}
	}
	...
	if( $version=="C" ) {
		show_page( "not_access.tpl" ,"你沒有權限使用此功能,2");
		exit;
	}
	else {
		show_page( "not_access.tpl" ,"You have No Permission!!,2");
		exit;
	}
}
...
function show_page_SST($msg)
{
	global $course_id;
        ...
        $tpl->assign("#!LINK!#","/php/Trackin/TGShowFrame2.php?loadtemp=1&course=" . $course_id . "&year=" . $_POST['year'] . "&term=" .$_POST['term']);
        $tpl->parse(BODY,"main");
        $tpl->FastPrint("BODY");
}
...
function upload_score(){
	global $SDB, $course_id, $report, $course_info, $count, $DB;
	...
	for($i=0;$i<sizeof($report);$i++){
		if($_POST["grade".$report[$i]['std_no']]!="" && $_POST["state".$report[$i]['std_no']]==1){
			...
			$cur = sybase_query($Qins, $cnx);	//測試用..不上傳--jp
			if(!$cur) {  
...
function upload_preview(){
	...
	for($i=0;$i<sizeof($report);$i++){
		if($_POST["grade".$report[$i]['std_no']]!="" && $_POST["state".$report[$i]['std_no']]==1){
		  ...
		  //$score_data = "<td><div align=\"center\"><input type='text' name='".'grade'.$report[$i]['std_no']."' value=\"".$std_grade."\" size=2 readonly></div></td>";
...
/*getSDB() : 取得此課為在職班或是一般班 , modified by intree*/
function getSDB(){
...
function show_page_d($page){
	...
	$grp = $course_info['gid'];
	// 取出所有選課學生學號及姓名
	// 因暑修 將 a31v_sel_class_summer_tea 修改為 a31v_sel_class_tea
	$Qs = "select cs.std_no, vstd.name, vstd.status from a31v_sel_class_summer_tea cs, a11vstd_rec_tea vstd where cs.cour_cd = '$cour_cd' and cs.grp = '$grp' and cs.std_no=vstd.id and cs.year = '$course_info[year]' and cs.term = '$course_info[term]' ".
	"union select cs.std_no, leave.name, leave.status from a31v_sel_class_summer_tea cs, a11vleave_rec_tea leave where cs.cour_cd = '$cour_cd' and cs.grp = '$grp' and cs.std_no=leave.id and cs.year = '$course_info[year]' and cs.term = '$course_info[term]' ".
...
*************************************************/
//98.08.12 因暑修資料修改將 year=97 term=2
...
/*********************************************************************************************************************************/
		...
		*/
		...
		if($data[$i]['status']=="休學")
			...
			//$Qs3 = "select now_dept, now_grade, now_class, abbrev from a11tstd_his_rec tstd, h0rtunit_a_ unit where tstd.std_no='$std_no' and tstd.now_dept=unit.cd and tstd.year='$course_info[year]' and tstd.term='$course_info[term]'";
			$Qs3 = "select now_dept, now_grade, now_class, abbrev from a11tstd_his_rec tstd, h0rtunit_a_ unit where tstd.std_no='$std_no' and tstd.now_dept=unit.cd and tstd.year='97' and tstd.term='2'";
		//
	...
	/****bluejam***data = qsort_multiarray ( $data, 'index', SORT_ASC );
	...
	for($i=1;$i<=$total;$i++){
		...
		$plist = $plist."<option value=\"TGUpload2.php?PHPSESSID=$PHPSESSID&action=$action&page=$i&course_id=$course_id\" $s>$i</option>";
	}
