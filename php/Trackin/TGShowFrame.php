...
if(isset($PHPSESSID) && session_check_teach($PHPSESSID) == 2){
	...
	$Qs = "select course.name as cname, course_group.name as gname from course, course_group where course.group_id=course_group.a_id and course.course_no = '$cno' and course.a_id = '$course_id' ";
	if ($result = mysql_db_query($DB,$Qs)){
		$row = mysql_fetch_array($result);
	...
	*/
	...
	if ($result = mysql_db_query($DB,$Qs)){
		if(($row = mysql_fetch_array($result))!=0){
			...
			show_page( "not_access.tpl" ,"目前尚未開放$course_info[year]學年$course_info[term]學期的成績上傳功能,請於開放後再行使用此功能!");
			exit;
