...
//
	...
	$course_id = $course;
	$Q1 = "SELECT u.a_id, u.id, u.name FROM user u, take_course tc WHERE u.a_id=tc.student_id AND tc.course_id='$course_id' and tc.credit = '1' and tc.year='$year' and tc.term = '$term' ORDER BY u.id";
//	echo $course_id."<br>";
			...
			*/
			...
			while ( $row1 = mysql_fetch_array($result1) ) {
				...
				while ( $row2 = mysql_fetch_array($result5) ) {
					if ( $row2[0] != "" && $row2[0] != "-1" && $row2[0] != null && $row2[1] != 0 ) {//20060608 jp
			...
			if($action == "upload"){
				...
				header("Location:TGShowFrame2.php?course=$course_id&year=$year&term=$term&PHPSESSID=$PHPSESSID");
			}
			//------------------------------------------------------------------------------------
			show_page_d();
		}
...
function show_page_d () {
	global $DB_SERVER, $DB_LOGIN, $DB, $DBC, $DB_PASSWORD, $version, $message, $course_id, $scordata, $countdata, $sel, $skinnum, $a_id, $id, $action, $work, $PHPSESSID;
				...
				if ( $k == $countdata[0][1] + 2 ) {
					...
					if ( $countdata[5][$k] == 0 ) {
						if ( $action == "modify_single" && $countdata[6][$k] == $a_id && $scordata[$j][0] == $id ) {
							...
							$tpl->assign(DATA,"<td><div align=\"center\"><a href=\"./TGQueryFrame1.php?action=modify_single&a_id=".$countdata[6][$k]."&id=".$scordata[$j][0]."&work=".$countdata[7][$k]."\" >.</a></div></td>");
						}
						else {
							if($scordata[$j][$k] < 60)
								$tpl->assign(DATA,"<td><div align=\"center\"><a href=\"./TGQueryFrame1.php?action=modify_single&a_id=".$countdata[6][$k]."&id=".$scordata[$j][0]."&work=".$countdata[7][$k]."\" ><font color=#ff0000 size=\"2\">".$scordata[$j][$k]."</font></a></div></td>");
							else
								$tpl->assign(DATA,"<td><div align=\"center\"><a href=\"./TGQueryFrame1.php?action=modify_single&a_id=".$countdata[6][$k]."&id=".$scordata[$j][0]."&work=".$countdata[7][$k]."\" ><font size=\"2\">".$scordata[$j][$k]."</font></a></div></td>");
						}
					}
					else {
						if($scordata[$j][$k] < 60)
