...
class XML
{
    ...
    */
    function load_file ( $file )
    {
		...
		$bad_chr = array("<br>" => "chr(br)", "<B>" => "chr(B)", "</B>" => "chr(/B)", "<I>" => "chr(I)", "</I>"=> "chr(/I)" , "<p>" => "chr(p)" );

                if ( !xml_parse($parser, strtr($content,$bad_chr), true) )
                {
    ...
    */
    function get_content ( $path )
    {
	    ...
	    // add it 2006-12-29
            $bad_chr = array("<br>" => "chr(br)", "<B>" => "chr(B)", "</B>" => "chr(/B)", "<I>" => "chr(I)", "</I>"=> "chr(/I)" , "<p>" => "chr(p)" );
            return strtr(stripslashes($this->nodes[$path]["text"]),array_flip($bad_chr));
        }
    }
