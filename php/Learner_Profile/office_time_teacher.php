<?php
	...
	********************/
	...
	global $DB_SERVER, $DB_LOGIN, $DB, $DB_PASSWORD, $course_id , $skinnum;
	...
	function update_data() {
		global $DB_SERVER, $DB_LOGIN, $DB, $DB_PASSWORD, $course_id, $skinnum, $version, $user_id;
		...
		else if($count<2)
		{
			...
			$Q0 = "SELECT a_id FROM user where id = '$user_id'";
			...
			$Q1 = "Update OfficeTime set Mon = '".$updatemon."', Tue = '".$updatetue."', Wed = '".$updatewed."', Thu = '".$updatetur."', Fri = '".$updatefri."', location ='".$_POST["location"]."', comment ='".$_POST["Message"]."' where teacher_id='".$row0['a_id']."'" ;
	...
	function show_page_d( $message="" ){
		global $DB_SERVER, $DB_LOGIN, $DB, $DB_PASSWORD, $course_id, $skinnum, $version, $user_id, $course_year, $course_term ;
		
		include("class.FastTemplate.php3");
		$tpl = new FastTemplate ( "./templates" );
		if( $version == "C" )
			$tpl->define ( array ( body => "office_time_teacher.tpl") );
		else
			$tpl->define ( array ( body => "office_time_teacher_E.tpl") );
		
		
		$Q0 = "SELECT * FROM user where id = '$user_id'";
		if ( !($result0 = mysql_db_query( $DB, $Q0  )) ) {
			$error = "資料庫讀取錯誤!!Q1";
			show_page ( "not_access.tpl", $error );
			exit;
		...
		$Q1 = "Select * From OfficeTime Where teacher_id='".$row0['a_id']."'";
		if ( !($result = mysql_db_query( $DB, $Q1  )) ) {
			$error = "資料庫讀取錯誤!!Q1";
			show_page ( "not_access.tpl", $error );
			exit;
		...
		$tpl->assign ( COURSE_TERM, $course_term );
