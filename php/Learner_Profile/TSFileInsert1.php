...
if ( isset( $upfile ) )
{
	...
	else if($upfile_size>$maxsize)
	{
		if( $version=="C" )
			$message = "上傳檔案超過$maxsize bytes!!!";
		else
			$message = "The upload file exceeds $maxsize bytes!!!";
		show_page_d($message);	
	}
	else
		handl_data();
...
function handl_data() {
	global $upfile, $version, $course_year, $course_term;
	$fp = fopen ($upfile, "r");
	while(!feof($fp))
	{
		$stdlist=$stdlist.fgets($fp,80);
	}
	fclose($fp);
	...
	if(!$testExpression || !$testEmpty)
	{
		...
		$temp1=implode("",$student);//合併所有的學生陣列值
		$temp2=explode("#",$temp1);//依照"#"來分割學生的帳號
		$recordCount=0;
		for($i=0;$i<count($temp2)-1;$i++)
		{
			$temp3=explode(",",$temp2[$i]);//分割每個學生的姓名跟帳號
			$stud_name[]=addslashes($temp3[0]);//姓名加"/"
			$stud_id[]=$temp3[1];//學生帳號
			$recordCount++;//計數
		}
		...
		$repeat_flag = false;
		$kk = "";
		$repeat=array_count_values($stud_id);//計算重複數
		...
		if($repeat_flag)
		{
			...
			{
				$Q1 = "Select * From user u ,take_course tc Where u.a_id=tc.student_id And tc.course_id='$course_id' And u.id='$stud_id[$i]' and tc.credit='1' and tc.year='$course_year' and tc.term = '$course_term'";
...
function add_stu ( $recordCount, $id, $name ) {
   	global $DB_SERVER, $DB_LOGIN, $DB,$DBC, $DB_PASSWORD, $course_id, $version, $course_year, $course_term;
	if ( !($link = mysql_pconnect($DB_SERVER,$DB_LOGIN,$DB_PASSWORD)) ) {
		$message = "資料庫連結錯誤!!";
		show_page_d ( $message );
		return;
	...
	for($i=0 ; $i < $recordCount ; $i++ )
	{
		...
		if(mysql_num_rows($resultOBJ) == 0 ) //如果沒有學生資料
		{
			...
			$Q3 = "Select group_id From course Where a_id='$course_id'"; //抓出課程群組
			...
			$row2 = mysql_fetch_array($resultOBJ); //新增課程關連
			...
			$Q5 = "Select a_id From homework"; //抓出學生的HomeWork
...
*/
		...
		else
		{
			...
			$Q9 = "Select a_id From exam";   //抓取考試a_id
...
function show_page_d ( $message = "" ) {
	...
	if($version=="C")
		$tpl->define(array(main => "TSFileInsert1_Ch.tpl"));
	else
		$tpl->define(array(main => "TSFileInsert1_En.tpl"));
