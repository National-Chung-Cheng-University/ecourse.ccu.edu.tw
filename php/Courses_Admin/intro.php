...
/*
	...
	$sql = "SELECT year, term 
			FROM this_semester 
			";
	if ( !($link = mysql_pconnect($DB_SERVER,$DB_LOGIN,$DB_PASSWORD)) ) {
		die("資料庫連結錯誤!!");		
	}
	else if ( !($result = mysql_db_query( $DB, $sql ) ) ) {
		die("資料庫讀取錯誤!!");		
	}
	$row = mysql_fetch_assoc($result);
	if($row['year'] == $_SESSION['course_year'] && $row['term'] == $_SESSION['course_term'])
		$upload_path = "../../$course_id/intro";
	else
		$upload_path = "../../echistory/$_SESSION[course_year]/$_SESSION[course_term]/$course_id/intro";
		*/
//var_dump($upload_path);
	...
	if (isset($upload) && $upload == "1") {
		if ( $file != "none" && $file != "") {
			...
			$new_file = "index".$ext;
			//if( fileupload ( $file, $upload_path, $new_file ) ) {
		...
		if(isset($FormType) == 0 || $FormType == 0){
			...
			for($ClassGoalNumberCounter = 1; $ClassGoalNumberCounter <= $ClassGoalNumber; $ClassGoalNumberCounter++){
				...
				//echo "ClassGoalNo:" . $ClassGoalNo[ $ClassGoalNumberCounter] . "<br>";	//for test
			}

			$CoreAbilitiesTotalNumber = 0;
			for($ClassGoalNumberCounter = 1; $ClassGoalNumberCounter <= $ClassGoalNumber; $ClassGoalNumberCounter++){
				//linsy@20140214，應以ClassGoal_Index做關聯，否則會得到錯誤結果
				...
				$SQL_Select = "SELECT * FROM IEET_CoreAbilities WHERE group_id = '$group_id' AND ClassGoal_Index = '$ClassGoal_Index[$ClassGoalNumberCounter]' ORDER BY CoreAbilitiesNo ASC";
				...
				for($CoreAbilitiesNumberCounter = 1; $CoreAbilitiesNumberCounter <= $CoreAbilitiesNumber[ $ClassGoalNumberCounter]; $CoreAbilitiesNumberCounter++){
					...
					//echo $content[$ClassGoalNumberCounter][$CoreAbilitiesNumberCounter] . "<br>";	//for test
					//linsy@20140214, 讓程式自己計算核心能力總數
					$TotalCoreNumber++;
				}
			}

			//指定核心力能總數
			/*  linsy@20140214, 讓程式自己計算，而非指定
			if($group_id==11) $TotalCoreNumber=11;
							...
							else if($group_id==19) $TotalCoreNumber=11;
			*/
			//echo 'Total Core Number: ' . $TotalCoreNumber. "<br>";
			...
			//linsy@20140214, 用一個計數器當$isChecked的Index(表示「已勾選」欄位)
			$CoreAbilitiesCounter = 1;
			for($ClassGoalNumberCounter = 1; $ClassGoalNumberCounter <= $ClassGoalNumber; $ClassGoalNumberCounter++){
				for($CoreAbilitiesNumberCounter = 1; $CoreAbilitiesNumberCounter <= $CoreAbilitiesNumber[ $ClassGoalNumberCounter]; $CoreAbilitiesNumberCounter++){
					...
					//linsy@20140214, 不指定group_id去指定固定欄位值，而改用自動判斷for all group_id
					if($isChecked[$CoreAbilitiesCounter] == "checked")
					...
					//echo "coreAbilitiesCounter = " . $CoreAbilitiesCounter . "<br/>";
					//echo "isChecked[CoreAbilitiesCounter] = " . $isChecked[$CoreAbilitiesCounter] . "<br/>";
					//echo "CoreAbilitiesList[" . $ClassGoalNumberCounter . "][" . $CoreAbilitiesNumberCounter . "]=" . $CoreAbilitiesList[$ClassGoalNumberCounter][$CoreAbilitiesNumberCounter] . "<br>";	//for test
...
function add_intro($intro)
{
	...
	else
	{
		...
		elseif(!($result = mysql_db_query($DB, $sql)))
		{
			$error = "資料庫寫入錯誤!!";
			return $error;
		}
...
function createForm( $ShowType, $disabled){	
	...
	//linsy@20140214, show ClassGoalNumber & ClassGoal的CoreAbilitiesNumber
	//echo "ClassGoalNumber:" . $ClassGoalNumber . "<br>";	//for test
	for($ClassGoalNumberCounter=1; $ClassGoalNumberCounter<=$ClassGoalNumber; $ClassGoalNumberCounter++)
	{
		//echo "ClassGoalNumber = " . $ClassGoalNumberCounter . ", ";
		//echo "CoreAbilitiesNumber = " . $CoreAbilitiesNumber[$ClassGoalNumberCounter] . "<br>";	//for test
	}
	...
	for($ClassTopicNumberCounter = 1; $ClassTopicNumberCounter<=$ClassTopicNumber; $ClassTopicNumberCounter++){
		...
		if($ShowType == 1)
		{
			...
			//linsy@20140214,動態產生課程大綱-單元主題的核心能力
			/*
				if($group_id==11){
					$TPL_REPLACE_ClassTopic_Content =	
						$TPL_REPLACE_ClassTopic_Content .
				...
				//
                        ...
                        $TPL_REPLACE_ClassTopic_Content =
                                $TPL_REPLACE_ClassTopic_Content .
                                "<input type=\"checkbox\" name=\"CoreAbility".$ClassTopicNumberCounter."_".$i."\" value=\"".$i."\" ".$checkedList[$ClassTopicNumberCounter][$i]." disabled>E".$i;
                }
			*/
			$i = 1;
			...
			for($ClassGoalNumberCounter=1; $ClassGoalNumberCounter<=$ClassGoalNumber; $ClassGoalNumberCounter++)
			{	
				//echo "ClassGoalNumberCounter = " . $ClassGoalNumberCounter . "<br>";
				for($CoreAbilitiesNumberCounter=1; $CoreAbilitiesNumberCounter<=$CoreAbilitiesNumber[$ClassGoalNumberCounter]; $CoreAbilitiesNumberCounter++)
				{	
					
					//echo "CoreAbilitiesNumberCounter = " . $CoreAbilitiesNumberCounter . "<br>";
							
					$TPL_REPLACE_ClassTopic_Content =	
					$TPL_REPLACE_ClassTopic_Content .
					"<input type=\"checkbox\" name=\"CoreAbility".$ClassTopicNumberCounter."_".$i."\" value=\"".$i."\" ".$checkedList[$ClassTopicNumberCounter][$i]." disabled>".$ClassGoalNumberCounter.".".$CoreAbilitiesNumberCounter;							
					$i++;
				}
			}
			//die();
			//echo 'i=' . $i;
			//linsy@20140214,動態產生課程大綱-單元主題的核心能力及是否勾選結束
		...
		{					
			...
			for($i=1; $i<=$TotalCoreNumber; $i++){
				...
				//
                                        ...
                                        $TPL_REPLACE_ClassTopic_Content =
                                              $TPL_REPLACE_ClassTopic_Content .
                                              "<input type=\"checkbox\" name=\"CoreAbility".$ClassTopicNumberCounter."_".$i."\" value=\"".$i."\" ".$checkedList[$ClassTopicNumberCounter][$i]." >E".$i;
                                }
				//
			}
			*/
			...
			{	
				...
				{	
					...
					$TPL_REPLACE_ClassTopic_Content =	
					$TPL_REPLACE_ClassTopic_Content .
					"<input type=\"checkbox\" name=\"CoreAbility".$ClassTopicNumberCounter."_".$i."\" value=\"".$i."\" ".$checkedList[$ClassTopicNumberCounter][$i].">".$ClassGoalNumberCounter.".".$CoreAbilitiesNumberCounter;							
					$i++;
				}
			}
			//die();
...
//var_dump($isChecked);
	...
	for($ClassGoalNumberCounter = 1; $ClassGoalNumberCounter<=$ClassGoalNumber; $ClassGoalNumberCounter++)
	{
		for($CoreAbilitiesNumberCounter = 1; $CoreAbilitiesNumberCounter<=$CoreAbilitiesNumber[$ClassGoalNumberCounter]; $CoreAbilitiesNumberCounter++)
		{
			...
			$TPL_REPLACE_CoreAbilities_TR =
				$TPL_REPLACE_CoreAbilities_TR . 
				"<input name=\"" . $CoreAbilities_NameTemp . "\" type=\"checkbox\" value=\"1\" onClick=\"" . $CoreAbilitiesOnClick_NameTemp . "()\" " .  " ". $isChecked[$i] . " disabled>"
