...
/*****刪除已退選的學生名單*********************
	devon 2006-02-16
	del_selected_lists.php
	
	show_ad2.tpl-->按鈕名稱：清除退選的學生名單
	將原本有選上但後來又退選的學生名單清楚
**********************************************/
require 'fadmin.php';
?>
...
<div><table>
    <tr> 
      <td> 
        <div>
          <font color=#000000>清除已退選學生名單!</font>
        </div>
      </td>
    </tr>
</table>
...
function add_student($SDB)
{
	...
	// 取出課程代碼
	if($SDB == "academic_gra")
		$Qs1 = "select c.course_no, c.course_id from course_no c, teach_course t, this_semester ts where t.year = ts.year and t.term = ts.term and c.course_id = t.course_id and (c.course_no like '___A%' or c.course_no like '___B%' or c.course_no like '___C%' or c.course_no like '___D%')";
	...
	while ($row1 = mysql_fetch_array($result1))
	{
		...
		// 從教學系統取出選課資料
		$Qs5 = "select student_id, id from user u,take_course tc where course_id = '$course_id' and u.a_id=tc.student_id and tc.credit=1";
