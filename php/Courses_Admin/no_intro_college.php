...
echo "<center>";
echo "<font color='blue' size='5'>".$row0['year']."學年第".$row0['term']."學期已上傳課程大網統計</font><br>";
...
if($status == 1)
{
	...
	*/
	...
	while($row2 = mysql_fetch_array($result2))
	{
		...
		if($self_count!=0)
		{
			$Q9 = "INSERT INTO temp_intro VALUES ( '$row2[a_id]', '$row2[name]', '$dept_total', '$self_count', '1', 0, '' )";
	...
	$Q8 = "update course_group, temp_intro set temp_intro.parent_id = course_group.parent_id where course_group.a_id = temp_intro.dep_id and temp_intro.is_upload = '1' ";
	if ( !($result8 = mysql_db_query( $DB, $Q8 ) ) )
	{
		$message = "$message - 資料庫讀取錯誤8!!";
	}

	$Q81 = "update course_group, temp_intro set temp_intro.parent_name = course_group.name where course_group.a_id = temp_intro.parent_id and temp_intro.is_upload = '1' ";
...
else
{
	...
	*/
	while($row2 = mysql_fetch_array($result2))
	{
		...
		if($self_count!=0)
		{
			$Q9 = "INSERT INTO temp_intro VALUES ( '$row2[a_id]', '$row2[name]', '$dept_total', '$self_count', '0', 0, '' )";
			if ( !($result9 = mysql_db_query( $DB, $Q9 ) ) )
			{
				$message = "$message - 資料庫讀取錯誤9!!";
			}			
...
$Q5 = "SELECT distinct parent_id, parent_name from temp_intro where is_upload='1' order by parent_id";
...
while($row5 = mysql_fetch_array($result5))
{
	...
	$Q12 = "SELECT sum(dep_total) dep_sum, sum(self_count) self_sum FROM temp_intro WHERE parent_id= $parent_id and is_upload='1' ";
	...
	$Q13 = "SELECT dep_name, dep_total, self_count FROM temp_intro WHERE parent_id = $parent_id and is_upload='1' ORDER BY dep_id";
