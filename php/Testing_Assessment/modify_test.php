...
if( isset($PHPSESSID) && (session_check_teach($PHPSESSID) >= 2) )
{
	...
	if($action == "modifynr")
	{
		$Q1 = "SELECT name, percentage,chap_num,CoreAbilities FROM exam WHERE a_id='$exam_id'";
	...
	else if($action == "modifytestnr")
	{
		...
		else
		{
			...
			if ( $test_type == "self_test" )
				...
				$Q1 = "UPDATE exam SET name='$test_name', chap_num='$chap_num', percentage='0', CoreAbilities='$str' WHERE a_id='$exam_id'";
			else
				$Q1 = "UPDATE exam SET name='$test_name', chap_num='$chap_num', percentage='$test_ratio', CoreAbilities='$str'  WHERE a_id='$exam_id'";
			if ( !($result = mysql_db_query( $DB.$course_id, $Q1 ) ) ) {
	...
	{
		$Q1 = "SELECT a_id, exam_id, type, question, selection1, selection2, selection3, selection4, answer, ismultiple, grade ,file_picture_type, file_av_type FROM tiku WHERE exam_id='$exam_id' order by a_id";
		if ( !($result = mysql_db_query( $DB.$course_id, $Q1 ) ) ) {
			show_page( "not_access.tpl" ,"資料庫讀取錯誤!!" );
		}
...
//--------------------------------------------------------------------
	...
	else if($action == "updatetest")
	{
		...
		if ( $type == 1 ) {
			$Q1 = "UPDATE tiku SET grade='$qgrade',question='$qtext',ismultiple='$cho',selection1='$selection1',selection2='$selection2',selection3='$selection3',selection4='$selection4',answer='$checkbox',answer_desc='$ans_link' , file_picture_type='$file_picture_type', file_av_type='$file_av_type' WHERE a_id='$a_id' AND exam_id='$exam_id'";
		}
		else if ( $type == 2 ) {
			$Q1 = "UPDATE tiku SET grade='$qgrade',question='$qtext',answer='$cho',answer_desc='$ans_link' , file_picture_type='$file_picture_type', file_av_type='$file_av_type' WHERE a_id='$a_id' AND exam_id='$exam_id'";
		}
		else if ( $type == 3 ) {
			$Q1 = "UPDATE tiku SET grade='$qgrade',question='$qtext',selection1='$selection1',selection2='$selection2',selection3='$selection3',selection4='$selection4',answer_desc='$ans_link',ismultiple='$cho',answer='$rownum' , file_picture_type='$file_picture_type', file_av_type='$file_av_type' WHERE a_id='$a_id' AND exam_id='$exam_id'";
		}
//--------------------------------------------------------------------
		//devon 問答題
		else if ( $type == 4 ) {
			$Q1 = "UPDATE tiku SET grade='$qgrade', question='$qtext',answer='$answer_qa', answer_desc='$ans_link', file_picture_type='$file_picture_type', file_av_type='$file_av_type' WHERE a_id='$a_id' AND exam_id='$exam_id'";
		}
//--------------------------------------------------------------------
		else {
		...
		if(is_file($picture_file)){
			if( !fileupload( $picture_file, "../../$course_id/exam", $a_id."_picture.".$file_picture_type ) ){
				show_page("not_access", "檔案寫入錯誤");
			}
			
		}		
		if(is_file($av_file)){
			if( !fileupload( $av_file, "../../$course_id/exam", $a_id."_av.".$file_av_type ) ){
				show_page("not_access", "檔案寫入錯誤");
			}
...
//--------------------------------------------------------------------
		//devon 問答題
			...
			$Q1 = "Insert into tiku ( exam_id, type, question, grade, answer_desc, file_picture_type, file_av_type ) values ( '$exam_id', '$type', '$qtext', '$qgrade', '$ans_link', '$file_picture_type', '$file_av_type' )";
		}
//--------------------------------------------------------------------
		else {
			show_page( "not_access.tpl" ,"題型錯誤!!" );
		...
		$Q1 = "update take_exam set grade = '-1', nonqa_grade = '-1' where exam_id = '$exam_id' and student_id = '$a_id'";
		if ( !($result = mysql_db_query( $DB.$course_id, $Q1 ) ) ) {
			echo ("資料庫更新錯誤1(retest)!!");
			exit;
		} 
//-------------------------------------------------------------------------
		$Q2 = "delete from qa where exam_id = '$exam_id' and student_id='$a_id'";
		if ( !($result2 = mysql_db_query( $DB.$course_id, $Q2 ))) {
			echo ("資料庫更新錯誤2(restes)!!");
			exit;
		}
//-------------------------------------------------------------------------
		showgrade();
	...
	//老師可以選擇某次測驗成績的公佈與否
	else if($action == "pubgrade")
	{
		$Q1 = "SELECT public from take_exam where exam_id='$exam_id'";
		$Q10 = "UPDATE take_exam SET public = '1' where exam_id='$exam_id'";
		$Q11 = "UPDATE take_exam SET public = '0' where exam_id='$exam_id'";
...
function show_page_d () {
	...
	$Q1 = "SELECT name, percentage, a_id,chap_num FROM exam WHERE is_online='1' ORDER BY name";
	if ( !($result = mysql_db_query( $DB.$course_id, $Q1 ) ) ) {
		show_page( "not_access.tpl" ,"資料庫讀取錯誤!!" );
	}
...
function modifynr () {
	global $DB, $message, $test_name,$exam_id, $test_ratio, $test_type, $version, $skinnum, $chap_num, $course_id, $CoreAbilities;
...
//start------------devon@2005-xx-xx--修改測驗時，可以更改測驗所屬章節--------	
...
//end------------devon@2005-xx-xx--修改測驗時，可以更改測驗所屬章節--------
...
function modifytest () {
	...
	if ( mysql_num_rows($result) == 0 ) {
		...
		if ( $rows['type'] == 1 ) {
			...
			$rows['selection1'] =  quotes($rows['selection1']) ;
			$rows['selection2'] =  quotes($rows['selection2']) ;
			$rows['selection3'] =  quotes($rows['selection3']) ;
			$rows['selection4'] =  quotes($rows['selection4']) ;
...
//--------------------------------------------------------------------
		//devon 2005-07-16 問答題
			...
			if($version == "C")
				...
				$tpl->assign(KIND,"Q & A");
			if($version == "C")
				$tpl->define(array(tail=>"edit_testqa.tpl"));
			else
				$tpl->define(array(tail=>"edit_testqa_E.tpl"));
			$tpl->assign(ANSWER_QA,$rows['answer']);
		}
//--------------------------------------------------------------------
		else {
			if($version == "C")
		...
		if ( !($type == "3" && ($rows['answer'] == "0" || $rows['answer'] == "") ) ) {
			//body
			$tpl->assign(QGRADE, $rows['grade']);
			$rows['question'] = quotes( $rows['question'] );
...
//----------------------------------------------------------------------
//devon 有問答題時的處理
			$QQ = "select question, answer, grade, grade_limit from qa where student_id='".$rows[a_id]."' and exam_id='$exam_id'";
			$resultQQ = mysql_db_query( $DB.$course_id, $QQ );

			$QQQ = "select * from tiku where exam_id='$exam_id' and type='4'";
			$resultQQQ = mysql_db_query( $DB.$course_id, $QQQ );
			$rowsQQQ = mysql_num_rows($resultQQQ);
			
			while( $rowsQQ = mysql_fetch_array($resultQQ) ) {
				$qaflag = 1;
				if ( $rowsQQ['answer'] == "" )
					$qaanswer = 0;
				else
			...
			if ( $rowsQQQ == 0 )
				...
				$tpl->assign(QA,"<a href=./modify_qa.php?id=$rows[id]&student_id=$rows[a_id]&exam_id=$exam_id>批改</a>"); //modified by carlyle 
//----------------------------------------------------------------------
...
function pubtest () {
	...
	{
		$year = date("Y")+$j;
		if ( ($j == 0 && $row1[3] != 1 && $row1[3] != 3 ) || ($year == substr( $row1[2], 0, 4 ) && $row1[3] == 1)  )
