<?php
//added by jimmykuo @ 20101025
//功能: 讓老師能觀看學生的作答紀錄,從runtest.php修改過來的
require 'fadmin.php';
...
//-------------------------------------------------------------------------	
	$Q3 = "select a_id from user where id='$stu_id'";
...
//-------------------------------------------------------------------
	...
	//}
	/*else if ( $guest == 1 ) {
		$Q2 = "SELECT a_id from user where id='$stu_id'";
		...
		$end_mo = (int) substr($row2[1],4,2);
		$end_d = (int) substr($row2[1],6,2);
		$end_h = (int) substr($row2[1],8,2);
		$end_m = (int) substr($row2[1],10,2);
		$now_y = (int) substr(date("YmdHi"),0,4);
		$now_mo = (int) substr(date("YmdHi"),4,2);
		$now_d = (int) substr(date("YmdHi"),6,2);
		$now_h = (int) substr(date("YmdHi"),8,2);
		...
		else if( $range > 0 && ($row2[1] != "00000000000000") && $row2[2] != 0) {
			if($version == "C")	
