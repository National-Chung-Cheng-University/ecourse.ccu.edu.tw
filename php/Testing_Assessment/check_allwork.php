...
//intree@2008-01-21, eAccelerator的cache page, 為了減少多人同時按同一頁面產生的delay
//eaccelerator_cache_page($_SERVER['PHP_SELF'].'?GET='.serialize($_GET).'&POST='.serialize($_POST).'&course='.$course_id.'&user_id='.$user_id, 5);
	...
	{
		$anchor=$_GET['anchor'];//added by jimmykuo @ 20100423 目的:按上傳評語,再回上一頁後可以回到原修改學生的錨點
		checkstu ($anchor);
	}
	...
	/**
       modified by devon 2005-05-02，一次上傳全部成績
		...
		$i=0;
		while($row_tmp=mysql_fetch_array($result0))
		//for( $i=0; $i<$row0; $i++ )
		{
			...
			{
				$anchor = $row_tmp['id']; //added by jimmykuo @ 20100423 目的:設定每位學生的錨點
				$flag=1;		
			}
...
/*
	modified by jimmykuo @ 20100423
	新增傳入參數anchor,目的是要在按給分時能回到原修改學生的錨點
*/
function checkstu ($anchor) {
	...
	if ( mysql_num_rows($result1) != 0 ) {
		...
		while ( $rows1 = mysql_fetch_array($result1) )
		{
			...
			$tpl->assign( COLOR , $color );
			$Q2 = "SELECT handin_time, grade, public, work FROM handin_homework WHERE student_id = '".$rows1[0]."' AND homework_id = '$work_id'";
			...
			if($rows2['handin_time'] == "" || $rows2['handin_time'] == "0000-00-00" )
			{
			...
			else
			{
				...
				if($rows2['handin_time'] > $rows3['due'])
					$tpl->assign(SDATE,"<font color='red'>" . $rows2['handin_time'] . "</font>");
				else
					$tpl->assign(SDATE,$rows2['handin_time']);
...
//*****************************
		...
		$tpl->assign(DEST,$anchor);//added by jimmykuo @ 20100423
		$tpl->parse(BODY,"main");
		$tpl->FastPrint("BODY");
	}
	else {
		...
		if ( is_dir( $work_dir ) ) {
			...
			while (( $file = $handle->read() ) ) {
				...
				// 除了 '.' '..'和非正常檔案(如目錄)之外的檔案輸出
					...
					$filelink = rawurlencode($file);
					//$tpl->assign("FILE_LINK", $work_dir."/".$file);
					$tpl->assign("FILE_LINK", $work_dir."/".$filelink);
